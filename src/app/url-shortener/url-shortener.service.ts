...
export class UrlShortenerService {
  ...
  shortenUrl(originalUrl: string) {
    ...
    this.http
      .post<{ message: string; data: Url }>(this.apiUrl, { originalUrl }, { headers })
      .subscribe({
        next: (response) => {
  ...
  resetUrlClicks(url: Url) {
    ...
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    this.http.put<{ message: string, data: Url }>(
      `${this.apiUrl}/${url.id}`,
      { clicks: 0 },
      { headers }
    ).subscribe({
  ...
  deleteUrl(url: Url): void {
    ...
    this.http.delete<{ message: string, url: Url}>(`${this.apiUrl}/${url.id}`)
      .subscribe(
        (response) => {
